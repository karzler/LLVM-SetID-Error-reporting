...
#include "llvm/MultiCompiler/Skein.h"       /* get the Skein API definitions   */
...
void    Skein1024_Process_Block(Skein1024_Ctxt_t *ctx,const uint8_t *blkPtr,size_t blkCnt,size_t byteCntAdd);

/*****************************************************************/
/*     Portable (i.e., slow) endianness conversion functions     */
uint64_t Skein_Swap64(uint64_t w64)
{    /* instantiate the function body here */
  static const uint64_t ONE = 1;              /* use this to check endianness */
...
{
  return ((uint8_t *) Skein_256_API_CodeSize) -
    ((uint8_t *) Skein_256_Init);
}
#endif
...
int Skein_512_InitExt(Skein_512_Ctxt_t *ctx,size_t hashBitLen,uint64_t treeInfo, const uint8_t *key, size_t keyBytes)
{
  uint64_t i;
  union
  {
...
int Skein1024_InitExt(Skein1024_Ctxt_t *ctx,size_t hashBitLen,uint64_t treeInfo, const uint8_t *key, size_t keyBytes)
{
  uint64_t i;
  union
  {
...
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
/* process the input bytes */
int Skein1024_Update(Skein1024_Ctxt_t *ctx, const uint8_t *msg, size_t msgByteCnt)
{
  size_t n;
...
{
  return ((uint8_t *) Skein1024_API_CodeSize) -
    ((uint8_t *) Skein1024_Init);
}
#endif
